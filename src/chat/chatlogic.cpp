...
ChatLogic::ChatLogic(){
    /*
     * start the chat manager instance since it is supposed to collect stats
     * through the entire lifetime of the chatbot
     */
    ChatbotManager::GetInstance().Start();
}

ChatLogic::~ChatLogic() {
    /*
     * stop the chat manager instance in order to store the running config stats
     */
    ChatbotManager::GetInstance().Stop();
}
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename) {
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it) {
        ...
        /* find the root node based on the node id */
        if ((*it)->GetID() == CHATBOT_START_NODE) {
            if (rootNode == nullptr) {
    ...
    ChatBot chatBotTemp("../images/chatbot.png");

    // add pointer to chatlogic so that chatbot answers can be passed on to the GUI
    chatBotTemp.SetChatLogicHandle(this);

    // add chatbot to graph root node
    chatBotTemp.SetRootNode(rootNode);
    rootNode->MoveChatbotHere(std::move(chatBotTemp));
}
...
void ChatLogic::SetChatMode(CHAT_MODE chatMode) {
    _panelDialog->SetChatMode(chatMode);
}
